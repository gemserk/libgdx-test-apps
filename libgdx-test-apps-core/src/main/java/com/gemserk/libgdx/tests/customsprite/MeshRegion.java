...
/**
 * Defines a polygon shape on top of a #TextureRegion for minimising pixel drawing. Can either be constructed through a .psh file from an external editor or programmatically through a list of vertices defining a polygon.
 * 
 * THIS STUFF IS WIP
 * 
...
public class MeshRegion {
	 ...
	 */
	public MeshRegion(TextureRegion region, FileHandle file) {
		this.region = region;
	 ...
	 */
	public MeshRegion(TextureRegion region, float[] vertices) {
		this.region = region;
		...
		for (int i = 0; i < triangulatedVectors.size(); i++) {
			...
			texCoords[i * 2] = u + (localVertices[i * 2] - region.getRegionX()) / region.getRegionWidth();
			texCoords[i * 2 + 1] = v + (1 - (localVertices[i * 2 + 1] - region.getRegionY()) / region.getRegionHeight());
		}
	}
	 ...
	 * 
	 * ------------ // Triangulated vertices data (x, y) in pixel coordinates with origin bottom-left, y-up v 230.0, 230.0, ... // UVs with origin top-left u 0.23, 0.123, ... -------------
	 * 
	...
	private float[] calculateAtlasTexCoords(float[] localTexCoords) {
		...
		float uvHeight = this.region.getV2() - this.region.getV();

		for (int i = 0; i < localTexCoords.length; i += 2) {
			localTexCoords[i] = this.region.getU() + localTexCoords[i] * uvWidth;
			localTexCoords[i + 1] = this.region.getV() + localTexCoords[i + 1] * uvHeight;
		}
